...
var tvRemote = function (word) {
  ...
  const pos = { x: 0, y: 0 };
  const getPos = (letter) => {
    const y = keyboard.findIndex(row => row.find(n => n === letter))
    const x = keyboard[y].findIndex(n => n === letter)
    return { x, y }
  };
  let moves = 0;
  for (let letter of word) {
    const newPos = getPos(letter);
    moves += Math.abs(pos.x - newPos.x) + Math.abs(pos.y - newPos.y) + 1;
    pos.x = newPos.x;
    pos.y = newPos.y;
  }
  return moves;
